...
class BayesianSpectralMixture(Kernel):
    ...
    def initialize_from_emp_spec(self, Q, x, y, fs):
        ...
        GMM = BayesianGaussianMixture(n_components=Q, init_params="random_from_data", max_iter=10000, weight_concentration_prior=0.001, covariance_type="full", weight_concentration_prior_type="dirichlet_distribution")
        GMM.fit(X=inv_spec)
        means = GMM.means_
        varz = GMM.covariances_
        weights = GMM.weights_
        threshold = 0.05
        means = means[weights>0.0001]
        varz = varz[weights>0.0001]
        weights = weights[weights>0.0001]
        if 0 > 1:
            print(len(weights), "components remaining", weights)
